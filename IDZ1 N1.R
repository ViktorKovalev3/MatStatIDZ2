...
################################################################
#Предположим, что исходные наблюдения являются выборкой из распределения геометрических распределений
#h.c.1. Построим оценку максимального правдоподбия параметра theta
library(maxLik)
LL<-function(t){sum(dgeom(x,t[1],log=TRUE))}
mme<-maxNR(LL,start=c(1))
estimate<-ml$estimate; estimate #оценка макс.правдоподобия

#h.c.3. Найдем смещение оценок
#Оценка не смещенная!

################################################################
#h.d. Построить асимптотический доверительный интервал уровня значимости a1 для параметра
#l на базе ОМП
ACI2<-array(dim=2)
ACI_delta<-qnorm(1-a1/2)
ACI2[1]<-mean-ACI_delta*sqrt((mean*(mean+1))/length(x)) #левая граница Д.И.
ACI2[2]<-mean+ACI_delta*sqrt((mean*(mean+1))/length(x)) #правая граница Д.И.
ACI2


################################################################
#h.e.1. Используя гистограмму частот, построить критерий значимости Xi^2 проверки простой 
#гипотезы согласия с распределением Пуассона с параметром l0.

#h.e.2. Проверить гипотезу на уровне значимости a1.

#h.e.3. Вычислить наибольшее значение уровня значимости, на котором еще нет оснований отвергнуть данную гипотезу.


################################################################
#h.f.1. Построить критерий значимости Xi^2 проверки сложной гипотезы согласия с распределением Пуассона. 

#h.f.2. Проверить гипотезу на уровне значимости a1. Вычислить наибольшее значение уровня значимости, 
#на котором еще нет оснований отвергнуть данную гипотезу.
